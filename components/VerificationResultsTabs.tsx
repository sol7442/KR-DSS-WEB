...
import EtsValidationReport from './reports/EtsValidationReport';

interface VerificationResultsTabsProps {
  result: VerificationResult;
}

type Tab = 'Simple' | 'Detailed' | 'Diagnostic Tree' | 'ETS Validation Report';
...
const VerificationResultsTabs: React.FC<VerificationResultsTabsProps> = ({ result }) => {
  ...
  const tabs: Tab[] = ['Simple', 'Detailed', 'Diagnostic Tree', 'ETS Validation Report'];

  const renderContent = () => {
    switch (activeTab) {
      case 'Simple':
        return <SimpleReport simpleReport={result.simpleReport} />;
      case 'Detailed':
        return <DetailedReport detailedReport={result.detailedReport} />;
      case 'Diagnostic Tree':
        return <DiagnosticTree diagnosticTree={result.diagnosticTree} />;
      case 'ETS Validation Report':
        return <EtsValidationReport etsValidationReport={result.etsValidationReport} />;
      default:
        return null;
    }
  };
